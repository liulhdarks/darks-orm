...
import darks.orm.annotation.sqlmap.page;
import darks.orm.annotation.sqlmap.pageSize;
import darks.orm.annotation.sqlmap.values;
...
public class ClassFactory
{
    ...
    private static final ConcurrentMap<String, InterfaceMethodData> mapIFaceMethod =
        new ConcurrentHashMap<String, InterfaceMethodData>();
     ...
     * @throws Exception
     */
    public static <T> void initClass(List<Class<T>> cs)
        throws Exception
    {
        for (Class<T> c : cs)
        {
     ...
     * @throws Exception
     */
    public static void initEntity(Collection<Class<?>> entitys)
        throws ClassNotFoundException
    {
        if (entitys.size() == 0)
            return;
    ...
    public static <T> InterfaceMethodData parseInterfaceClass(String id, Method method)
    {
        ...
        synchronized (method)
        {
            ...
            data =
                new InterfaceMethodData(id, pageIndex, pageSizeIndex, valuesIndex, sql, resultClass, cacheId,
                    attribute, alias, autoCache, autoCascade);
            for (int i = 0; i < method.getParameterTypes().length; i++)
            {
                try
                {
                    ...
                    if (ap.annotationType().equals(page.class))
                    {
                        pageIndex = i;
                    }
                    else if (ap.annotationType().equals(pageSize.class))
                    {
                        pageSizeIndex = i;
                    }
                    else if (ap.annotationType().equals(values.class))
                    {
                        valuesIndex = i;
                    }
                    else if (ap.annotationType().equals(Param.class))
                    {
                        Param param = (Param)ap;
                        data.addArgument(i, param.value());
                    }
                }
                catch (Exception e)
                {
            ...
            data.setPageSizeIndex(pageSizeIndex);
            data.setValuesIndex(valuesIndex);
            if (queryType == QueryType.SingleType)
            {
                data.setQueryEnumType(QueryEnumType.Object);
            }
            else if (queryType == QueryType.PageType)
            {
                data.setQueryEnumType(QueryEnumType.Page);
            }
            else
            {
                data.setQueryEnumType(QueryEnumType.List);
            }
     ...
     * @throws Exception
     */
    public static <T> EntityData parseClass(Class<T> cls)
        throws SessionException
    {
        ...
        {
            throw new SessionException("ClassFactory::parseClass ClassNotFoundException " + e.toString(), e);
        }
        ...
        throws SessionException
    {
        ...
        { // 是否为主键
            ...
            PrimaryKeyData pkdata = new PrimaryKeyData(pkid.type(), pkid.seq(), pkid.feedBackKey(), pkid.select());
     ...
     * @throws Exception
     */
    public static <T> String getPrimaryKeyName(Class<T> c)
        throws SessionException
    {
     ...
     * @throws Exception
     */
    public static <T> Object getPrimaryKeyValue(Class<?> c, T obj)
        throws SessionException
    {
        FieldData data = getPrimaryKey(c);
        if (data != null)
     ...
     * @throws Exception
     */
    public static FieldData getPrimaryKey(Class<?> c)
        throws SessionException
    {
     ...
     * @throws ClassNotFoundException
     */
    public static <T> String getFieldNames(Class<T> c)
        throws SessionException
    {
